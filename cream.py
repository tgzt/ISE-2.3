...
class ERS(object):
		...
		if not is_valid:
			...
			if found_endpoint['ns3:searchResult']['@total'] == '1':
				resp = self.ise.get('{0}/config/endpoint/{1}'.format(
						self.url_base, found_endpoint['ns3:searchResult']['ns3:resources']['ns5:resource']['@id']))
				if resp.status_code == 200:
					result['success'] = True
	...
	def get_user(self, user_id):
		...
		if found_user['ns3:searchResult']['@total'] == '1':
			resp = self.ise.get('{0}/config/internaluser/{1}'.format(
					self.url_base, found_user['ns3:searchResult']['ns3:resources']['ns5:resource']['@id']))
			if resp.status_code == 200:
				result['success'] = True
	...
	def delete_user(self, user_id):
		...
		if found_user['ns3:searchResult']['@total'] == '1':
			user_oid = found_user['ns3:searchResult']['ns3:resources']['ns5:resource']['@id']
